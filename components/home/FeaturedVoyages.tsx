...
export default function FeaturedVoyages() {
  ...
  const getYear = (d) => { if (!d) return ''; const m = String(d).match(/\d{4}/); return m ? m[0] : ''; };
  ...
  return (
    <section style={{ background: '#070707', padding: '0' }}>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '80px 40px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <Link href="/voyages" style={{ fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(245,240,232,0.4)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px', fontFamily: 'system-ui' }}
          onMouseEnter={e => e.currentTarget.style.color = '#c4962a'}
          onMouseLeave={e => e.currentTarget.style.color = 'rgba(245,240,232,0.4)'}>
          Tout voir <span style={{ display: 'block', width: '32px', height: '1px', background: 'currentColor' }} />
        </Link>
      </div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 40px 80px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3px' }}>
        ...
        {main && (
          <Link href={`/voyages/${main.slug || main.id}`} style={{ display: 'block', textDecoration: 'none', position: 'relative', overflow: 'hidden', aspectRatio: '3/4', gridRow: 'span 2' }}
            onMouseEnter={() => setHovered(main.slug || main.id)} onMouseLeave={() => setHovered(null)}>
            ...
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '32px' }}>
              ...
              <h3 style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: 'clamp(1.6rem,3vw,2.4rem)', fontWeight: 300, color: '#f5f0e8', fontStyle: 'italic', margin: '0 0 12px', lineHeight: 1.15 }}>{main.title}</h3>
            ...
            <Link key={v.slug||v.id} href={`/voyages/${v.slug||v.id}`} style={{ display: 'block', textDecoration: 'none', position: 'relative', overflow: 'hidden' }}
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '24px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                  ...
                  <h3 style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: 'clamp(1.2rem,2.5vw,1.7rem)', fontWeight: 300, color: '#f5f0e8', fontStyle: 'italic', margin: 0, lineHeight: 1.2 }}>{v.title}</h3>
                </div>
                <div style={{ width: '36px', height: '36px', border: `1px solid ${hovered === (v.slug||v.id) ? '#c4962a' : 'rgba(255,255,255,0.2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .3s', flexShrink: 0 }}>
                  <span style={{ color: hovered === (v.slug||v.id) ? '#c4962a' : 'rgba(255,255,255,0.4)', fontSize: '14px' }}>→</span>
                </div>
              </div>
            </Link>
          ))}
      ...
      {others[2] && (
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 40px 80px' }}>
          <Link href={`/voyages/${others[2].slug||others[2].id}`} style={{ display: 'block', textDecoration: 'none', position: 'relative', overflow: 'hidden', height: '220px' }}
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', padding: '0 40px', gap: '32px' }}>
              ...
              <div>
                ...
                <h3 style={{ fontFamily: '"Cormorant Garamond",serif', fontSize: '2rem', fontWeight: 300, color: '#f5f0e8', fontStyle: 'italic', margin: 0 }}>{others[2].title}</h3>
              </div>
